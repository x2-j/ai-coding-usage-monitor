...
- `CodexCliProviderAdapter` is opt-in. When `codex_tracking_enabled` is true, it scans Codex session JSONL files for `last_token_usage` counters and extracts only token totals, timestamps, and model names. It does not read `auth.json`, logs, SQLite rows, prompts, responses, tool output, project paths, or credential material.
